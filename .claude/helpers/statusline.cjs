...
function getV3Progress() {
  ...
  let domainsCompleted = dddData && dddData.completed !== undefined
    ? Math.min(5, dddData.completed)
...
function getSecurityStatus() {
  ...
  if (auditData) {
    ...
    if (!auditDate) {
      return { status: 'PENDING', cvesFixed: 0, totalCves: auditData.totalCves || 3 };
    }
  ...
  // 3 known CVEs tracked in ADRs: CVE-1 (outdated deps), CVE-2 (weak password), CVE-3 (hardcoded creds)
  return {
    status: scanCount > 0 ? 'SCANNED' : 'PENDING',
    cvesFixed: 0,
    totalCves: 3,
  };
}
...
function getSystemMetrics() {
  ...
  // Sub-agents from swarm activity (agent_count tracks real spawned agents)
  let subAgents = 0;
  const activityData = readJSON(path.join(CWD, '.claude-flow', 'metrics', 'swarm-activity.json'));
  if (activityData && activityData.swarm && activityData.swarm.agent_count) {
    subAgents = activityData.swarm.agent_count;
  }
...
// Single-line output that never collapses
function generateOneline() {
  ...
  // RuFlo + user
  parts.push(c.bold + c.brightPurple + '\u258A RuFlo V' + pkgVersion + c.reset);
  parts.push((swarm.coordinationActive ? c.brightCyan : c.dim) + '\u25CF' + c.reset + c.brightCyan + ' ' + git.name + c.reset);
  ...
  if (git.gitBranch) {
    ...
    if (changes > 0) {
      ...
      if (git.modified > 0) ind += c.brightYellow + '~' + git.modified;
      if (git.untracked > 0) ind += c.dim + '?' + git.untracked;
      branchStr += ' ' + ind + c.reset;
    }
  ...
  parts.push(c.brightYellow + '\uD83E\uDD16' + agentsColor + swarm.activeAgents + c.reset + '/' + swarm.maxAgents);

  // Hooks
  if (hooks.enabled > 0) {
    parts.push(c.brightBlue + '\uD83E\uDE9D ' + c.brightGreen + hooks.enabled + c.reset);
  }
  ...
  // Patterns / vectors
  if (agentdb.vectorCount > 0) {
    parts.push(c.brightCyan + '\uD83D\uDCBE' + agentdb.vectorCount + (agentdb.hasHnsw ? '\u26A1' : '') + c.reset);
  } else if (progress.patternsLearned > 0) {
    const pk = progress.patternsLearned >= 1000 ? (progress.patternsLearned / 1000).toFixed(1) + 'k' : String(progress.patternsLearned);
    parts.push(c.brightYellow + '\uD83D\uDCDA' + pk + c.reset);
  }
...
} else if (process.argv.includes('--oneline')) {
  console.log(generateOneline());
} else {
  console.log(generateOneline());
}
