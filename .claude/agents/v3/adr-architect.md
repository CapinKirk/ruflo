...
hooks:
  pre: |
    ...
    # Search for related ADRs via CLI
    npx @claude-flow/cli@latest memory search --query "architecture decision record" --namespace decisions --limit 10 2>/dev/null || true
    # Load project ADR context
    if [ -d "docs/adr" ] || [ -d "docs/decisions" ] || [ -d "v3/implementation/adrs" ]; then
      echo "Found existing ADR directory"
      ls docs/adr/ v3/implementation/adrs/ v3/docs/adr/ 2>/dev/null | head -20
    fi
  post: |
    echo "ADR documentation complete"
    # Store ADR outcome in memory via CLI
    npx @claude-flow/cli@latest memory store --key "adr-latest" --value "ADR task completed" --namespace decisions 2>/dev/null || true
    # Record successful outcome for routing learning
    npx @claude-flow/cli@latest hooks post-task --task-id "adr-task" --success true 2>/dev/null || true
---
